      ...
      <div className="w-full max-w-md rounded-[16px] border border-slate-200 bg-white shadow-md p-6">
        ...
        {sent ? (
          <div className="mt-4 rounded-[10px] border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
            If an account exists for <span className="font-medium">{email}</span>, a reset link has been sent. Please check your inbox and spam.
          </div>
        ) : (
          <form onSubmit={onSubmit} className="mt-4 space-y-4">
            ...
            <p className="text-center text-sm text-slate-600">
              Remembered your password? <a href={`/login?callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-slate-900 underline">Back to login</a>
            </p>
          </form>
        )}
      </div>
