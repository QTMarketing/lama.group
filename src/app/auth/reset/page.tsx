...
export default function ResetPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await signIn("credentials", { redirect: true, callbackUrl, email: user.email, password: p1, mode: "password" });
    } catch {
      setErr("Reset failed. The link may be invalid or expired."); setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-md rounded-[16px] border border-slate-200 bg-white shadow-md p-6">
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-4">
          ...
          <p className="text-center text-sm text-slate-600">
            Back to <a href={`/auth?mode=login&callbackUrl=${encodeURIComponent(callbackUrl)}`} className="text-slate-900 underline">login</a>
          </p>
        </form>
      </div>
    </main>
